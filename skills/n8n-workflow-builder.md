...
→ Slack/iMessage: Notify the operator if lead score ≥ 8
```
...
→ iMessage: Daily summary to the operator ("Found X leads, Y hot")
```
...
4. If max iterations reached: executor saves diagnosis report and alerts the operator
...
4. Notify the operator: "Workflow [name] is live. Trigger: [X]. Monitors: [Y]."
