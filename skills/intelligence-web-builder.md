...
**Primary option: FAL.ai via Claude in Chrome MCP** — requires an active FAL account. Navigate to `https://fal.ai` and use Flux or compatible model for image gen. FAL also supports Kling for video generation (see 3B), making the entire Phase 3 fully automated.
...
7. **Copy to site folder** via osascript:
   ```applescript
   do shell script "cp ~/Downloads/'Gemini_Generated_Image_[hash].png' $HOME/jarvis/owners-inbox/hero-before.png"
   ```
...
Use FAL.ai via Claude in Chrome MCP — requires an active FAL account. FAL supports Kling video generation natively.
...
2. **Start if not running** (via osascript):
   ```applescript
   do shell script "cd $HOME/jarvis/owners-inbox && nohup python3 -m http.server 8099 > /tmp/httpserver.log 2>&1 &"
   ```
3. **Open in Chrome:**
   ```
...
| FAL.ai (Flux) via Chrome MCP | Generate before/after hero frames — requires an active account |
