...
| Workstream | What |
|------------|------|
| **Tool research** | Run `mcp-discovery` to confirm every required MCP is loaded or installable. Note any auth steps the operator will need to do. |
...
**Show the PRD to the operator and wait for explicit approval.** Do not proceed to build until the operator says "yes" or edits it. This is the highest-leverage gate in the pipeline — five minutes here saves an hour of rebuilds later.
...
- **Auto-installing MCPs.** The operator approves connector installs. Always suggest, never silently install.
