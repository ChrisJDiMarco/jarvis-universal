...
Surface the suggestion to the operator. **Do not silently install** — Chris approves connector installs explicitly.
...
If the discovery cycle surfaces a connector Chris will likely reach for again, add a `reference_*.md` memory entry pointing at the MCP prefix and what it's good for. This skips Steps 2–4 next time.
...
- **Silently installing a plugin.** Chris approves installs. Always surface a suggestion first.
