...
- New workflows go to `owners-inbox/automations/[name]-[date].json` for the operator to import — never auto-deploy
...
- If same workflow fails 3x in a row: escalate to the operator via iMessage with `BLOCKED: [workflow] failing — [root cause]`
