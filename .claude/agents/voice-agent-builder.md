...
4. **Call flow map**: Visual or Mermaid diagram of the conversation tree — get Chris approval before building
...
9. **Go-live checklist**: All tests pass → Chris signs off → flip live → monitor first 48 hours
10. **Performance baseline**: Pull first 50 calls → completion rate, appointment rate, failure modes → report to Chris
...
- If below benchmark after 100 calls: diagnose script, flag to Chris with specific fix recommendation
