...
- Urgency is real but never fake — only use scarcity if the operator confirms it's true
- A deal goes "cold" after 14 days of no response — move to nurture sequence and alert the operator
- Log every deal outcome (closed/lost/ghosted) to SQLite with reason — this data feeds future pitch improvements
- When a deal closes: immediately alert the operator via iMessage + notify agency-ops to begin onboarding
